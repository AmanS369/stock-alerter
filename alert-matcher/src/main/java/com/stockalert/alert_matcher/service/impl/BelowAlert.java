...
@Service("BELOW")
@Slf4j
public class BelowAlert implements IAlertStrategy {
    ...
    public TriggeredAlertBatchDto getTriggeredAlerts(
        ...
        String activeKey = "alerts:" + symbol + ":BELOW";
        String processingKey = activeKey + ":processing";

        DefaultRedisScript<List> script = new DefaultRedisScript<>();
        script.setLocation(new ClassPathResource("scripts/claim_alerts_below.lua"));
        ...
        if (alerts != null && !alerts.isEmpty()) {
            log.info("⬇ BELOW triggered {} alerts for {}", alerts.size(), symbol);
